...
<div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; margin: 30px 0;">
    ...
    <div style="border: 1px solid #ddd; padding: 20px; border-radius: 5px;">
        ...
        $current_membership_id = wp_insert_post(array(
            ...
            'post_content' => '<h2>Current Year Membership (2024-2025)</h2>
...
<p><a href="/dashboard">← Back to Dashboard</a> | <a href="/dashboard/current-membership">Current Members</a> | <a href="/dashboard/member-directory">Member Directory</a></p>',
        ...
        $directory_page_id = wp_insert_post(array(
            ...
            'post_content' => '<h2>SWCA Member Contact Directory</h2>
...
<p><a href="/dashboard">← Back to Dashboard</a> | <a href="/dashboard/current-membership">Current Members</a> | <a href="/dashboard/historical-membership">Historical View</a></p>',
        ...
        $stats_page_id = wp_insert_post(array(
            ...
            'post_name' => 'stats',
            'post_content' => '<h2>SWCA Membership Statistics</h2>
[swca_stats]
...
<p><a href="/dashboard">← Back to Dashboard</a> | <a href="/dashboard/current-membership">Current Members</a> | <a href="/dashboard/fiscal-table">Fiscal Analysis</a></p>',
        ...
        $fiscal_page_id = wp_insert_post(array(
            ...
            'post_content' => '<h2>SWCA Fiscal Year Membership Analysis</h2>
[swca_fiscal_table]
...
<p><a href="/dashboard">← Back to Dashboard</a> | <a href="/dashboard/current-membership">Current Members</a> | <a href="/dashboard/email-dashboard">Email Management</a></p>',
        ...
        $settings_page_id = wp_insert_post(array(
            ...
            'post_name' => 'settings',
            'post_content' => '<h2>⚙️ SWCA Settings & Configuration</h2>
...
function swca_create_members_table() {
    ...
    $sql = "CREATE TABLE $table_name (
        ...
        status_2025_2026 varchar(50) DEFAULT '' NOT NULL,
        status_2023_2024 varchar(50) DEFAULT '' NOT NULL,
        membership_month varchar(50) DEFAULT '' NOT NULL,
        ...
        on_email_list varchar(200) DEFAULT '' NOT NULL,
        categories text DEFAULT '' NOT NULL,
        tags text DEFAULT '' NOT NULL,
        notes longtext DEFAULT '' NOT NULL,
        import_date datetime DEFAULT CURRENT_TIMESTAMP,
        reconciled varchar(10) DEFAULT '' NOT NULL,
        on_bettes_list varchar(10) DEFAULT '' NOT NULL,
        created_date datetime DEFAULT CURRENT_TIMESTAMP,
        PRIMARY KEY (id),
        INDEX idx_email (email_1),
        INDEX idx_name (first_name, last_name),
        INDEX idx_status_2024_2025 (status_2024_2025),
        INDEX idx_status_2023_2024 (status_2023_2024)
    ) $charset_collate;";
...
// Simple immediate import without AJAX
function swca_process_immediate_import() {
    // Verify nonce
    ...
    try {
        ...
        // Use WordPress file handling or simple copy for development
        if (is_uploaded_file($uploaded_file['tmp_name'])) {
            if (!move_uploaded_file($uploaded_file['tmp_name'], $zip_path)) {
                wp_die('Failed to move uploaded file');
            }
        } else {
            // Fallback for development/testing - direct copy
        ...
        $overwrite_data = isset($_POST['overwrite_data']);
        $import_settings = isset($_POST['import_settings']);
        
        // 1. Create backup if requested
        if ($backup_existing) {
            $backup_dir = $import_dir . '/backup_' . time();
            wp_mkdir_p($backup_dir);
            
            $backup_sql = "-- SWCA Database Backup " . date('Y-m-d H:i:s') . "\n";
        ...
        if (file_exists($members_file)) {
            ...
            if (($handle = fopen($members_file, 'r')) !== FALSE) {
                ...
                while (($data = fgetcsv($handle)) !== FALSE) {
                    if (count($data) >= count($headers)) { // Ensure we have enough columns
                        // Build member data using headers to map columns correctly
                        $member_data = array();
                        for ($i = 0; $i < count($headers); $i++) {
                            if (isset($data[$i]) && $headers[$i] !== 'id') { // Skip 'id' field
                                $member_data[$headers[$i]] = $data[$i];
                            }
                        }
        ...
        // 4. Run schema updates to ensure all tables exist
        ...
        <div class="wrap">
            ...
            <div class="card" style="max-width: 800px; padding: 20px;">
                ...
                <p>
                    ...
                    <a href="<?php echo site_url('/dashboard/member-directory'); ?>" class="button">👥 View Members</a>
                </p>
            </div>
        </div>
        <?php
...
function swca_member_profile_handler($atts) {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'add_note' && current_user_can('swca_view_dashboard')) {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'update_categories' && current_user_can('swca_view_dashboard')) {
    ...
    $notes = $wpdb->get_results($wpdb->prepare("
        ...
        LEFT JOIN {$wpdb->prefix}users u ON n.created_by = u.ID
        WHERE n.member_id = %d 
...
function swca_email_dashboard_handler($atts) {
    ...
    $emails = $wpdb->get_results($wpdb->prepare("
        ...
        LEFT JOIN {$wpdb->prefix}users a ON e.approved_by = a.ID
...
function swca_import_data_with_progress() {
    ...
    <div class="wrap">
        ...
        <div class="card" style="max-width: 800px; padding: 20px;">
            ...
            <div id="progress-container" style="margin: 20px 0;">
                <div class="progress-bar" style="width: 100%; background: #f1f1f1; border-radius: 10px; padding: 3px;">
                    <div id="progress-fill" style="width: 0%; background: #2196f3; height: 20px; border-radius: 7px; transition: width 0.5s;"></div>
                </div>
                <p id="progress-text" style="margin: 10px 0; font-weight: bold;">Starting import...</p>
            </div>
            ...
            <div id="success-section" style="display: none; margin-top: 20px; padding: 20px; background: #e7f7d3; border-radius: 5px; border: 1px solid #4caf50;">
                <h3>✅ Import Complete!</h3>
                <p><strong>Your data has been successfully imported.</strong></p>
                <a href="<?php echo site_url('/dashboard'); ?>" class="button button-primary">🏠 Go to Dashboard</a>
                <a href="<?php echo admin_url('admin.php?page=swca-export'); ?>" class="button">🔄 Import More Data</a>
            </div>
    ...
    (function() {
        ...
        function showSuccess() {
            ...
            updateProgress(100, 'Import complete - all data restored!');
        }
        
        // Start the import process
        addLog('📥 Starting SWCA data import...');
        updateProgress(5, 'Uploading and extracting files...');
        ...
        const formData = new FormData();
        formData.append('action', 'swca_process_complete_import');
        formData.append('swca_import_nonce', '<?php echo wp_create_nonce('swca_export_action'); ?>');
        formData.append('import_file', fileInput.files[0]);
        formData.append('backup_existing', <?php echo isset($_POST['backup_existing']) ? 'true' : 'false'; ?>);
        formData.append('overwrite_data', <?php echo isset($_POST['overwrite_data']) ? 'true' : 'false'; ?>);
        formData.append('import_settings', <?php echo isset($_POST['import_settings']) ? 'true' : 'false'; ?>);
        ...
        .then(data => {
            if (data.success) {
                ...
                addLog('📊 Imported: ' + data.data.summary);
                showSuccess();
            } else {
                addLog('❌ Import failed: ' + data.data.error);
                showError(data.data.error || 'Unknown error occurred');
            }
        })
        .catch(error => {
        ...
        let progressInterval = setInterval(() => {
            if (progress < 85) {
                progress += Math.random() * 15;
                if (progress >= 20 && progress < 40) {
                    updateProgress(progress, 'Extracting ZIP package...');
                    addLog('📦 Extracting uploaded files...');
                } else if (progress >= 40 && progress < 60) {
                    updateProgress(progress, 'Importing member data...');
                    addLog('👥 Processing member records...');
                } else if (progress >= 60 && progress < 80) {
                    updateProgress(progress, 'Restoring database...');
                    addLog('🗄️ Importing database tables...');
                } else if (progress >= 80) {
                    updateProgress(progress, 'Finalizing import...');
                    addLog('⚙️ Applying settings and configurations...');
                }
            } else {
                clearInterval(progressInterval);
            }
...
function swca_ajax_process_complete_import() {
    ...
    try {
        ...
        if (is_uploaded_file($uploaded_file['tmp_name'])) {
            ...
            // Fallback for development/testing - direct copy
            if (!copy($uploaded_file['tmp_name'], $zip_path)) {
                wp_send_json_error(array('error' => 'Failed to copy uploaded file'));
            }
        }
