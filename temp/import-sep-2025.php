<?php
/**
 * Import September 2025 bank transactions
 */
...
echo "Importing September 2025 Bank Transactions\n";
echo str_repeat("=", 80) . "\n\n";

$csv_file = '/home/swca/scripts/501c3PO/bank-2025-september.csv';
...
// Verify by counting
$count_sep_2025 = $wpdb->get_var("SELECT COUNT(*) FROM wp_swca_bank_transactions WHERE YEAR(post_date) = 2025 AND MONTH(post_date) = 9");
echo "Total September 2025 transactions in database: $count_sep_2025\n";

$balance_check = $wpdb->get_row("
    SELECT
        MIN(post_date) as first_date,
        MAX(post_date) as last_date,
        (SELECT balance FROM wp_swca_bank_transactions WHERE YEAR(post_date) = 2025 AND MONTH(post_date) = 9 ORDER BY post_date ASC, id ASC LIMIT 1) as first_balance,
        (SELECT balance FROM wp_swca_bank_transactions WHERE YEAR(post_date) = 2025 AND MONTH(post_date) = 9 ORDER BY post_date DESC, id DESC LIMIT 1) as last_balance
    FROM wp_swca_bank_transactions
    WHERE YEAR(post_date) = 2025 AND MONTH(post_date) = 9
");
