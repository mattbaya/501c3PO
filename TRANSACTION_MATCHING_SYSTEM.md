...
### 14.1 Low GF→Stripe Match Rate

**Possible Causes:**
- Stripe sync incomplete (run longer date range)
- Timestamp tolerance too strict (increase from 30s to 60s)
- Gravity Forms webhook failing

**Solution:**
```sql
-- Check unmatched GF payments
SELECT * FROM swca_gf_addon_payment_transaction gf
...
### 14.2 Low Bank→Stripe Match Rate

**Possible Causes:**
- Missing payout_id data
- Bank CSV missing Stripe deposits
- Algorithm grouping incorrectly

**Solution:**
```sql
-- Check payout data
SELECT COUNT(*) FROM swca_c3_stripe_transactions WHERE payout_id IS NULL;

-- If count > 0, re-sync Stripe or manually link:
UPDATE swca_c3_stripe_transactions st
JOIN swca_c3_stripe_balance_transactions bt
  ON st.balance_transaction_id = bt.balance_txn_id
SET st.payout_id = bt.payout_id
...
### 14.3 Duplicate Matches

**Possible Cause:** Algorithm run multiple times without clearing old matches
...
DELETE m1 FROM swca_c3_transaction_matches m1
INNER JOIN swca_c3_transaction_matches m2
WHERE m1.stripe_transaction_id = m2.stripe_transaction_id
  AND m1.bank_transaction_id = m2.bank_transaction_id
  AND m1.id > m2.id;
```
